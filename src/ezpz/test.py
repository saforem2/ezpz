...
class Trainer:
    ...
    def __post_init__(self):
        ...
        if wandb is not None and not WANDB_DISABLED and self.rank == 0:
            run = ezpz.setup_wandb(project_name='ezpz.test')
            assert (
    ...
    def train_step(self) -> dict:
        self.train_iter += 1
        fwd = self._forward_step()
        dtb = self._backward_step(fwd['loss'])
        metrics = {
            'train': {
    ...
    def finalize(self) -> Dataset:
        ...
        dset = self.history._nested_dict_to_dataset(
            self.history.history['train'],
            sep='/',
        )
        # dataset = self.history._finalize_dataset(
        #     'train',
        #     dataset=dset,
        #     run_name='ezpz.test',
        #     dataset_fname='train',
        ...
        #     save=(self.rank == 0),
        #     plot=(self.rank == 0),
        #     outdir=Path(os.getcwd()).joinpath('outputs', 'ezpz.test'),
        # )
    ...
    def train(self) -> Dataset | None:
        ...
        #     self.finalize()
        #     if self.rank == 0
        #     else None  # self.history.get_dataset(warmup=self.config.warmup)
        # )
