...
1. **Install `ezpz`[^uvi]**:
    ...
    # if you need torch and/or mpi4py, add them with:
    uv pip install "git+https://github.com/saforem2/ezpz[torch,mpi]"
    ```
...
`ezpz` provides a few useful commands to help with distributed launches and
...
- **Scheduler smarts:** detects PBS/Slurm automatically;  
  ...
  `ezpz launch` is preferred; the legacy `ezpz-launch` shim prints a red deprecation notice.
  Use `ezpz launch --print-source` to locate the CLI shim.
