...
## High-Level Flow
- **CLI → Launcher:** User calls `ezpz-launch` (or `python -m ezpz.launch`). CLI parses args/env, detects scheduler, and prepares a launch command.
...
- `launch.py` — CLI + command builder; scheduler-aware vs. local fallback paths.
- `jobs.py` — shared job metadata helpers; scheduler-neutral layer.
- `pbs.py` / `slurm.py` — scheduler-specific discovery (hostfiles, env vars, GPU counts).
- `dist.py` — torch distributed bootstrap; device/backend selection; rank/env plumbing.
...
- `utils/` — shell helpers, env management (`yeet`/`tar`), lazy imports, job env save/load.
- `integrations.py` / `hf_trainer.py` / `cria.py` — ecosystem hooks (HF, custom runners).
