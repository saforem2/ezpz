...
[^schedulers]:
    By default, this will detect if we're running behind a job
...
- 📝 [`ezpz.examples.*`](../../examples/index.md): Scalable and _ready-to-go_!
  ...
  - ??? tip "Running Examples"
          ...
          Any of the examples below can be launched with
  ...
  - ??? tip "🤗 HF Integration"
          ...
          1. `ezpz.examples.`
            {[`fsdp_tp`][ex-fsdp], [`diffusion`][ex-diffusion],
            [`hf_trainer``][ex-hf-trainer]} all support arbitrary
             🤗 Hugging Face
          ...
          1. [`ezpz.examples.hf_trainer`][ex-hf-trainer]
             supports arbitrary combinations of (compatible)
             `transformers.from_pretrained` models, and HF Datasets (with
...
[^distributed-history]:
    The `ezpz.History` class automatically computes
