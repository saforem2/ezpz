...
We recently introduced a mechanism for `launch`-ing python _from_ python.
...
to _launch_ [`ezpz/test_dist.py`](src/ezpz/test_dist.py) across all available resources in your {PBS, Slurm} job.
...
- For example, on any of the ALCF systems, it will automatically:
  ...
  - With this information, we can construct the full `mpiexec ...` command needed to launch our distributed application, e.g.:
